...
def format_docs(docs) -> str: 
    """Format retrieved documents into a structured text block for the prompts."""
    if not docs:
...
def build_chain(query: str):
    """Build the generation chain with retrieval , prompt, LLM and parser."""
    ...
    chain = (
        {"context":retriever | format_docs, "questions": RunnnablePassthrough()}
        | prompt
        | llm
...
def invoke_chain(query: str, debug: bool = False):
    """Invoke the generation chain and return the response along with evaluation scores."""
